...
const NavBar = () => {
  ...
  const links = [
    ...
    // Hide rss  { id: 2, name: locale.NAV.RSS, to: '/feed', show: true, external: true },
        ...
        {links.map(
          link =>
            link.show && (
              <li
                key={link.id}
                className="px-2 py-1.5 block ml-1 md:ml-4 rounded-lg text-gray-600 transition duration-500 ease-in-out hover:bg-white hover:rounded-lg hover:text-gray-800 hover:border-gray-300 hover:shadow-lg dark:text-gray-50 nav"
              >
...
export default function Header ({ navBarTitle, fullWidth }) {
  ...
  return (
    <>
      ...
      <div
        className={`sticky-nav font-serif group m-auto w-full h-6 flex flex-row justify-between items-center mb-2 md:mb-12 py-8 bg-opacity-60 ${
