        ...
        {links.map(
          link =>
            link.show && (
              <li
                key={link.id}
                className="px-2 py-1.5 block ml-1 md:ml-4 rounded-lg text-gray-600 transition duration-300 ease-in-out custom-header-btn-hover-shadow dark:text-gray-50 nav"
              >
...
export default function Header ({ navBarTitle, fullWidth }) {
  ...
  return (
    <>
      ...
      <div
        className={`sticky-nav font-serif group m-auto w-full h-4 md:h-6 flex flex-row justify-between items-center mb-2 md:mb-12 py-8 bg-opacity-60 ${
